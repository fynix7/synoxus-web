...
export default async function handler(req, res) {
    ...
    try {
        ...
        while (hasMore) {
            ...
            for (let i = 0; i < outliers.length; i += AI_BATCH_SIZE) {
                ...
                try {
                    ...
                    console.error('Error processing batch with AI:', e.message);
                    ...
                    batch.forEach(o => {
                        batchResults.push({
                            ...
                            generated_example: 'AI Analysis Failed - Check API Key'
                        });
                    });
                }
            ...
            // Limit to prevent timeout (Vercel has 10s limit on hobby, 60s on pro)
            if (page >= 2) {
                hasMore = false; // Process max 100 outliers per run to avoid timeout
            }
        }
        ...
        return res.status(200).json({
            ...
            message: `Processed ${processedCount} outliers. Run again for more.`
        });
